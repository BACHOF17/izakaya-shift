...
export default function StaffPunchHistoryPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/punch-history?month=${selectedMonth}`)
      .then(r => r.json())
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-5">
          <div className="text-center mb-4">
            ...
            <span className="text-gray-500 ml-1 text-sm">日出勤</span>
          </div>
          ...
          {records.length === 0 ? (
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-gray-400 font-medium px-3">
                <span>日付</span>
                <span>出勤</span>
                <span>退勤</span>
              </div>
              {records.map((r, i) => (
                <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className={`text-sm ${r.clock_in ? 'text-green-600' : 'text-gray-300'}`}>
                    {formatTime(r.clock_in)}
                  </span>
                  <span className={`text-sm ${r.clock_out ? 'text-red-500' : 'text-gray-300'}`}>
